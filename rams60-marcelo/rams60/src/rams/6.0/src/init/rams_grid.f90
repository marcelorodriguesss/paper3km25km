...
!  Fill RAMS inverse distance arrays.
      ...
      dxm(i,j) = 1. / earthdist_lc(xtn(i+1,ngrid),xtn(i,ngrid)  &
         ,ymn(j,ngrid),ymn(j,ngrid),polelat,polelon,stdlat1,stdlat2)
      dxt(i,j) = 1. / earthdist_lc(xmn(i,ngrid),xmn(i-1,ngrid)  &
         ,ytn(j,ngrid),ytn(j,ngrid),polelat,polelon,stdlat1,stdlat2)
      dxu(i,j) = 1. / earthdist_lc(xtn(i+1,ngrid),xtn(i,ngrid)  &
         ,ytn(j,ngrid),ytn(j,ngrid),polelat,polelon,stdlat1,stdlat2)
      dxv(i,j) = 1. / earthdist_lc(xmn(i,ngrid),xmn(i-1,ngrid)  &
         ,ymn(j,ngrid),ymn(j,ngrid),polelat,polelon,stdlat1,stdlat2)
      dym(i,j) = 1. / earthdist_lc(xmn(i,ngrid),xmn(i,ngrid)  &
         ,ytn(j+1,ngrid),ytn(j,ngrid),polelat,polelon,stdlat1,stdlat2)
      dyt(i,j) = 1. / earthdist_lc(xtn(i,ngrid),xtn(i,ngrid)  &
         ,ymn(j,ngrid),ymn(j-1,ngrid),polelat,polelon,stdlat1,stdlat2)
...
!  divided by earth distance
      ...
      fmapm(i,j) = dxm(i,j) * (xtn(i+1,ngrid) - xtn(i,ngrid))
      fmapt(i,j) = dxt(i,j) * (xmn(i,ngrid) - xmn(i-1,ngrid))
      fmapu(i,j) = dxu(i,j) * (xtn(i+1,ngrid) - xtn(i,ngrid))
      fmapv(i,j) = dxv(i,j) * (xmn(i,ngrid) - xmn(i-1,ngrid))
