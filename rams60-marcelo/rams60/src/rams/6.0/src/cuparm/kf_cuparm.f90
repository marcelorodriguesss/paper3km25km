...
!
! The modifications to the convective trigger function formulation
...
!-----------------------------------------------------------
   SUBROUTINE KF_eta_PARA (I, J,                           &
                      U0,V0,T0,QV0,P0,PPBS,DZQ,            &
                      W0AVG1D,W0AVG1DLT,TKE,TOPO,DT,DX,DXSQ,    &
                      IMPHYS,ICLOUD,IRAIN,IPRIS,ISNOW,IDIFFK, &
                      XLV0,XLV1,XLS0,XLS1,   &
                      CP,R,G,DQDT,DQIDT,DQCDT,DQRDT,DQSDT, &
...
! LOCAL VARS
      ...
      REAL    :: FBFRC,P300,DPTHMX,THMIX,QMIX,ZMIX,PMIX,   &
                 ...
                 ZLCL,WKL,WKLORIG,WKLTOPO,WKLELEV, WKLVAPOR, &
                 TOPOCOEF,ELEVCOEF,     &
                 WABS,TRPPT,WSIGNE,DTLCL,GDT,WLCL,WLCLMIN, WLCLELEV, &
...
!
   ...
   real    :: var1,var2,qui,qqtot,qtot
   INTEGER :: NTSD

!      INTEGER :: KX,K,KL
! abb modified
      INTEGER :: KX,K,KL,nkj,ij,iptb
...
!        DP(K)=PPBS(K)-PPBS(K+1)                                              ! mods for 1-d
...
!   (e.g. ratio DX/25.0 instead of DX/25.E3)
          ...
          if(w0avg1dlt(k).le.0.and.w0avg1dlt(KLCL).le.0) then
          ...
          if(w0avg1dlt(k).le.0.and.w0avg1dlt(KLCL).gt.0) then
            WKLORIG=(W0AVG1D(K)+(W0AVG1D(KLCL)-W0AVG1D(K))*DLP)*DX/25.E3-WKLCL
            WKLTOPO=(-TOPOCOEF*W0AVG1DLT(KLCL)+TOPOCOEF*W0AVG1DLT(K))*DLP*(DX/25.E3)*(DX/33.E3)
          endif

          if(w0avg1dlt(k).gt.0.and.w0avg1dlt(KLCL).le.0) then
            WKLORIG=(W0AVG1D(K)+(W0AVG1D(KLCL)-W0AVG1D(K))*DLP)*DX/25.E3-WKLCL
            WKLTOPO=(-TOPOCOEF*W0AVG1DLT(K))*(DX/25.E3)*(DX/33.E3)          
          endif

          if(w0avg1dlt(k).gt.0.and.w0avg1dlt(KLCL).gt.0) then
            WKLORIG=(W0AVG1D(K)+(W0AVG1D(KLCL)-W0AVG1D(K))*DLP)*DX/25.E3-WKLCL
            WKLTOPO=(-TOPOCOEF*W0AVG1DLT(K)+(-TOPOCOEF*W0AVG1DLT(KLCL))*DLP)*(DX/25.E3)*(DX/33.E3)
          endif
...
! Original code for convective trigger function
...
! approximately as a step function with an elevation threshold of 1500 m
...
!   FOR RAMS ACTIVATE THIS OPTION FOR NO MICROPHYSICS OR DUMPBUCKET
          ...
          IF(IMPHYS.LT.2.OR.IMPHYS.GT.5.OR.ICLOUD.EQ.0.OR.IRAIN.EQ.0)THEN
           RLC=XLV0-XLV1*TG(K)
           RLS=XLS0-XLS1*TG(K)
           CPM=CP*(1.+0.887*QG(K))
...
!   OR USING DUMPBUCKET SCHEME (IMPHYS = 6)
...
! THE FOLLOWING OPTION DOES NOT APPLY TO RAMS, SO IT'S COMMENTED OUT

!          ELSEIF(IAGGR.EQ.0)THEN
!
...
!-----------------------------------------------------------------------
   ...
   INTEGER ::    IPTB,ITHTB
   real :: var1, var2,a1,a2,a3,caca,pis
!-----------------------------------------------------------------------
...
!        IF(qqtot.ge.DQ)THEN
! abb When changed to "gt" the program run OK in cluster
...
!!     INTEGER, INTENT(OUT), PARAMETER :: kts=1,kte=50               ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: ids=1,ide=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: jds=1,jde=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: kds=1,kde=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: ims=1,ime=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: jms=1,jme=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: kms=1,kme=50               ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: its=1,ite=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: jts=1,jte=1                ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: IMPHYS=4                  ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: NTST=10                    ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: P_QI=1                    ! mods for 1-d version
!!     INTEGER, INTENT(OUT), PARAMETER :: P_QS=0                    ! mods for 1-d version
!
!!  REAL,     DIMENSION( ims: , kms: , jms: ) , INTENT(OUT) ::    &  ! mods for 1-d version
!   REAL,     DIMENSION( ims:ime,kts:kte,jms:jme) , INTENT(OUT) :: & ! mods for 1-d version
!                                                          RTHCUTEN, &
!                                                          RQVCUTEN, &
!                                                          RQCCUTEN, &
!                                                          RQRCUTEN, &
...
!                                                          RQSCUTEN
!
!!  REAL ,   DIMENSION( ims: , kms: , jms: ) , INTENT(OUT) :: W0AVG   ! mods for 1-d version
!   REAL , DIMENSION(ims:ime,kts:kte,jms:jme) , INTENT(OUT) :: W0AVG  ! mods for 1-d version
!
!!  INTEGER, DIMENSION( ims: , jms: ), INTENT(INOUT):: NCA        ! mods for 1-d version
!   INTEGER, DIMENSION( ims:ime , jms:jme ), INTENT(INOUT):: NCA  ! mods for 1-d version
!
!   INTEGER :: i, j, k, itf, jtf, ktf
!   REAL    :: SVP1,SVP2,SVP3,SVPT0
!
!   jtf=min0(jte,jde-1)
!!  ktf=min0(kte,kde-1)                                           ! mods for 1-d version
!   itf=min0(ite,ide-1)
!   ktf=kte                                                       ! mods for 1-d version
!
!   DO j=jts,jtf
!   DO k=kts,ktf
!   DO i=its,itf
