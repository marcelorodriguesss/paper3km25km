...
do ns=1,num_oda_sfc
   ...
   !   print*, mynum,'---ij:',ns,ikobs(nobs),jkobs(nobs)
   ...
   tkobs(nobs)=oval
   !if(ns==433.or.ns==444.or.ns==434.or.ns==681) &
   !   print*, mynum,'---tt:',ns,oval
   !if(mynum==0.and.nobs==681) print*,'oval:',mynum,nobs,oval,oda_sfc_obs(ns)%temp(1:ntimes)
   !if(mynum==0.and.nobs==681) print*,'oval:',mynum,nobs,oval,oda_sfc_obs(ns)%time(1:ntimes)
   !if(mynum==1.and.nobs==70) print*,'oval:',mynum,nobs,oval,oda_sfc_obs(ns)%time(1:ntimes)
   !if(mynum==1.and.nobs==70) print*,'oval:',mynum,nobs,oval,oda_sfc_obs(ns)%temp(1:ntimes)
   call get_obs_value(ntpast,time,oda_sfc_obs(ns)%time(1),ntimes  &
...
subroutine sfc_obs_convert(ng,nobs)
...
do ns=1,nobs
   ...
   if (tkobs(ns) > -998. .and. pkobs(ns) > -998.) then
        !print*,mynum,'---tconv:',ns,tkobs(ns),pkobs(ns)
