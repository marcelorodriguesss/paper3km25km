...
character (len=256) :: hnamedat,prefix
character (len=2) :: cng
character (len=6) :: cgrid
...
open(iunhd,file=fnames_nud(nnud),form='FORMATTED',status='OLD')
...
! Go through files and get all grids' TOPT's
do ngr = 1, ngridsh
   write(cgrid,'(a2,i1,a3)') '-g',ngr,'.h5'
   nc=len_trim(fnames_nud(nnud))
   hnamedat=fnames_nud(nnud)(1:nc-9)//trim(cgrid)
   inquire(file=trim(hnamedat),exist=there)

   call shdf5_open(hnamedat,'R')
...
read_loop: do nvh=1,nvbtab
   ...
   ngrec = hr_table(nvh)%ngrid
   if (ngrec /= ngold) then
      write(cgrid,'(a2,i1,a3)') '-g',ngrec,'.h5'
      nc=len_trim(fnames_nud(nnud))
      hnamedat=fnames_nud(nnud)(1:nc-9)//trim(cgrid)
      inquire(file=trim(hnamedat),exist=there)

      if (ngold /= 0) call shdf5_close()
      call shdf5_open(hnamedat,'R')
      ngold = ngrec
   endif
