...
integer :: i,j,k,lv,ifm,loop,n,iunit,iyyo,immo,iddo,ihho
real, external :: rs
...
if(isversion.eq.1) then
   rewind iunit
   read(iunit,*) iyy,imm,idd,ihh,nprz,nprx,npry,xswlon,xswlat,gdatdx,gdatdy
   read(iunit,*) (levpr(n),n=1,nprz)
   inproj=1
  ...
  ! [-180.,180.].
   ...
   do j = 1,n2
      do i = 1,n1
         ...
         if(gry < 1 .or. gry > fnpry) then
            print*,'Model grid point latitude must be'
            print*,'    inside pressure data area'
            print*,'    for a limited-area pressure dataset.'
            print*,'    Run will stop.'
            print*,'ifm,i,j,gry,glat,xswlat,gdatdy'  &
                   ,ifm,i,j,gry,gglat,xswlat,gdatdy
            stop 'pressure_stage: outside p-data lat'
         endif
         if(idatelin == 1 .and. gglon < xswlon) gglon=gglon+360.
         grx=(gglon-xswlon)/gdatdx+1.
         if (iglobew == 0) then
            if(grx < 1 .or. grx > fnprx) then
               print*,'Model grid point longitude must be'
               print*,'    inside pressure data area'
               print*,'    for a limited-area pressure dataset.'
               print*,'    Run will stop.'
               print*,'  ifm,i,j,grx,glon,xswlon,gdatdx'  &
                        ,ifm,i,j,grx,gglon,xswlon,gdatdx
               stop 'pressure_stage: outside p-data lon'
            endif
         endif
      enddo
   enddo
