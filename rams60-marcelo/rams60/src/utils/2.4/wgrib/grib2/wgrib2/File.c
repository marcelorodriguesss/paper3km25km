...
int f_bin(ARG1) {
    unsigned int i;
    if (mode == -1) {
        if ((*local = (void *) fopen(arg1, file_append ? "ab" : "wb")) == NULL) {
...
/*
 * HEADER:100:ieee:output:1:write big-endian IEEE data to X
 */
...
int f_text(ARG1) {
    ...
    if (mode == -1) {
        if ((*local = (void *) fopen(arg1,file_append ? "a" : "w")) == NULL) {
	    fatal_error("Could not open %s", arg1);
	}
        decode = 1;
    }
	...
	for (i = 0; i < ndata; i++) {
	   /* fprintf((FILE *) *local, text_format, data[i]);
	    ...
	    fprintf(stdout, "%g\n", data[i]);
            //fprintf(stdout, ((i+1) % text_column) ? " " : "\n");
        }
    }
    return 0;
}
