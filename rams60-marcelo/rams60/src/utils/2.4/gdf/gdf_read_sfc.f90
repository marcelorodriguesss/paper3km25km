...
character(len=256) :: line,tokens(100)
integer :: ntok
...
real :: v1,v2,v3,v4,v5,v6,vv
integer :: nv,ierr
real,external :: gdf_get_sfc_val, rs, td
...
do nv=1,nvars
   ...
   elseif(trim(cvar)=='dewptc') then  
      ...
      if (ierr == 1) then
         ! Missing dewpoint, try to convert rel humidity
         v1=gdf_get_sfc_val('TEMPERATURE','C',ierr)
         v2=gdf_get_sfc_val('RELATIVE_HUMIDIT','pct',ierr)
         v3=gdf_get_sfc_val('STN_PRES','Pa',ierr)
         if(v1 > -998. .and. v2 > -998. .and. v3 > -998.) then
            v4 = rs(v3,v1+273.16) * v2 * .01
            varn(nv)=td(v3,v4)-273.16
            ierr = 0
         endif
      else
         varn(nv) = v1
   ...
   elseif(trim(cvar)=='relhum') then  
      ...
      varn(nv)=100.*min(1.,max(0.  &
               ,rs(v3*100.,v2+273.16)/rs(v3*100.,v1+273.16)))
...
!print*,'||',trim(varn),'||',trim(header(curfile)%sfc_string(nv)),'||'
   if (trim(varn) == trim(header(curfile)%sfc_string(nv))) then
      ...
      !print*,'|| Found:',nv,' ',trim(varn),gdf_get_sfc_val
      return
   endif
enddo
