...
integer, parameter :: max_up_levs=1000,max_upa_vars=12
...
type obs_header
   character(len=80) :: head_string(max_head_vars),head_units(max_head_vars) &
...
integer :: ngood_gdf
character(len=32) :: good_id_gdf(maxgood_gdf)

! Bad station variables
integer, parameter :: maxbad_gdf=1000
integer :: nbad_gdf
character(len=32) :: bad_id_gdf(maxbad_gdf)
...
read(iunit,*,end=100,err=100) marker, iver
read(iunit,*,end=100,err=100)
read(iunit,*,end=100,err=100) token
...
!subroutine gdf_missing_sfc(sfcobs)
!
!type(gdf_sfc_obs) :: sfcobs
!
!real, parameter :: missing=-999.
!
!! Define a surface obs with missing values
!
!sfcobs%id='MISSING'
!sfcobs%lat=missing
!sfcobs%lon=missing
