...
subroutine latlon_ps(n1,n2,np1,np2,np3,nvar,ipos,aps,app  &
...
real :: aps(n1,n2,np3,*),app(np1,np2,np3,*),glon(n1,n2),glat(n1,n2)

! Array for global dataset interpolation
real, allocatable :: psglob(:,:)

real :: grx,gry,gglon
integer :: nv,i,j,k,iglob,igl,jgl,ip

! Determine psglob structure. If we are global in any direction,
!    add 2 extra rows on each boundary.
iglob = 0
if (iglobew > 0 .or. iglobs > 0 .or. iglobn > 0) then
   iglob = 1 ; igl = np1+4 ; jgl = np2+4
   allocate (psglob(-1:np1+2,-1:np2+2))
else
   iglob = 0 ; igl = np1 ; jgl = np2
   allocate (psglob(igl,jgl))
endif
...
do j = 1,np2
      ...
      if (iglob == 1) then
         psglob(   -1,1:np2) = psglob(np1-1,1:np2)
         psglob(    0,1:np2) = psglob(np1  ,1:np2)
         psglob(np1+1,1:np2) = psglob(    1,1:np2)
         psglob(np1+2,1:np2) = psglob(    2,1:np2)
         
         psglob(-1:np1+2,   -1) =    psglob(-1:np1+2,  np2-1)
         psglob(-1:np1+2,    0) =    psglob(-1:np1+2,  np2  )
         psglob(-1:np1+2,np2+1) =    psglob(-1:np1+2,      1)
         psglob(-1:np1+2,np2+2) =    psglob(-1:np1+2,      2)
      endif
      ...
      do j = 1,n2
         do i = 1,n1
           gry = (glat(i,j) - xswlat) / gdatdy + 1.

            gglon = glon(i,j)
            if (idatelin == 1 .and. gglon < xswlon)  &
                gglon = glon(i,j) + 360.
            ...
            call gdtost_llps2(psglob(-1,-1),igl,jgl,grx,gry,aps(i,j,k,nv),ip)
...
subroutine gdtost_llps(a,ix,iy,stax,stay,staval,ip)
implicit none
integer :: ix,iy,ip
!real :: a(0:ix,0:iy),r(4),scr(4),stax,stay,staval
real :: a(1:ix,1:iy),r(4),scr(4),stax,stay,staval

! Special version that starts data array dimensioned at (0,0)
...
111   continue
yy=stay-fiym2
if(ip==1) print'(a,i4,10f10.3)','r:',ii,r,yy,scr(ii)
call binom(1.,2.,3.,4.,r(1),r(2),r(3),r(4),yy,scr(ii))
100   continue
xx=stax-fixm2
call binom(1.,2.,3.,4.,scr(1),scr(2),scr(3),scr(4),xx,staval)
if (ip==1) &
   print '(a,10f10.3)','**gd:',scr(1),scr(2),scr(3),scr(4),stax,xx,staval
return
end

!     ******************************************************************

subroutine gdtost_llps2(a,ix,iy,stax,stay,staval,ip)
implicit none
integer :: ix,iy,ip
real :: a(1:ix,1:iy),r(0:3),scr(0:3),stax,stay,staval
...
!     AND STATION COLUMN.
!     VALUES GREATER THAN 1.0E30 INDICATE MISSING DATA.
integer :: iy1,iy2,ix1,ix2,i,j
real :: fx,fy,yy,xx


! ix1,iy1 - grid point to start 4x4 stencil
! ix2,iy2 - range of stencil
! stencil is given coordinates 1. 2. 3. 4. for binom
!  so the grid point location is converted to location between 2 and 3
!    for all binom calls
! xx,yy - float values of the location between 2 and 3
...
do i = ix1, ix2
   ...
   yy=stay-fy
   call binom(1.,2.,3.,4.,r(0),r(1),r(2),r(3),yy,scr(i-ix1))
   !if(ip==1) print'(a,i4,10f10.3)','r:',i,r,yy,scr(i-ix1)
enddo
xx=stax-fx
call binom(1.,2.,3.,4.,scr(0),scr(1),scr(2),scr(3),xx,staval)
!if (ip==1) &
!   print '(a,10f10.3)','**gd:',scr(0),scr(1),scr(2),scr(3),stax,xx,staval
...
!***************************************************************************
!***************************************************************************
subroutine ps_ps (n1,n2,np1,np2,n3,arrout,arrin  &
                  ,glatin1,glonin1,glatout,glonout  &
                  ,polelatin,polelonin,ddxin,ddyin)
implicit none
integer :: n1,n2,np1,np2,n3
real :: polelatin,polelonin,ddxin,ddyin,glatin1,glonin1
real :: arrout(n1,n2,n3),arrin(np1,np2,n3),glonout(n1,n2),glatout(n1,n2)
...
call ll_xy(glatin1,glonin1,polelatin,polelonin,xxi,xxj)

do i=1,n1
   do j=1,n2

      ! RAMS lat/lon coordinates on the incoming polar stereo coord
      call ll_xy(glatout(i,j),glonout(i,j),polelatin,polelonin,xi,xj)
      ri = 1. + (xi - xxi) / ddxin
      rj = 1. + (xj - xxj) / ddyin

      if(ri < 1 .or. ri > np1 .or. rj < 1 .or. rj > np2) then
         print*,' gridpoint outside Polr Ster data coverage'
         print*,'   polar i,j,lat,lon-',i,j,glatout(i,j),glonout(i,j)
         print*,'   computed datafile-relative point:',ri,rj
         stop 'in sub ps_ps'
      endif
      ...
      ! horizontally interpolate variables
         do k=1,n3
            call gdtost(arrin(1,1,k),np1,np2,ri,rj,arrout(i,j,k))
         enddo
