...
integer, save :: maxmem,maxslab,maxnxyg
integer :: memsiz4
...
save  nllat,nllon,dllat,dllon,swllat,swllon
...
print*
print*,'in read_RAMS-',action,'-',cgrid,'-',ctime,'-',len(ctime)
...
print*,'ipl 1:'
   ...
   do ng=1,ngrids
      ...
      memsiz4=maxmem
   enddo
   !print*,'compute memory',maxmem,memsiz4
   ...
   if(allocated(volt))    deallocate(volt);    allocate(volt(maxmem))
   ...
   if(allocated(topta))    deallocate(topta);    allocate(topta(maxslab))
   if(allocated(pctlnd))  deallocate(pctlnd);  allocate(pctlnd(maxslab))
   if(allocated(voltsl))  deallocate(voltsl);  allocate(voltsl(maxslab))
...
if(alat1(nngd)==0. .and. (action(1:4)=='DUMP' .or.  &
   ...
   call RAMS_fill_fld (idims(1),idims(2),idims(3)  &
                      ...
                      ,icfile,icgrid,icvar  &
                      ,cdname,cdunits,ivtype,icoor,topta)
...
! Read land fraction and grid cell volume
if(icfile.eq.1.or.icgrid.eq.1)  then 
   call RAMS_varlib ('land',idims(1),idims(2),1,nngd  &
                    ,pctlnd,arrb,flnm(1:lenhf-9)  &
                    ,cdname,cdunits,icoor)
   if(if_adap ==1) call RAMS_fill_fld (idims(1),idims(2),idims(3)  &
                    ,volt,arrb,arrbb,coor,topo  &
                    ,flnm(1:lenhf-9),nngd,'volt',itrans,nplevs  &
...
if(action(1:5).eq.'SPACE'.and.cvar(1)(1:3).ne.'map') then
   ...
   if((icfile.eq.1.or.icgrid.eq.1.or.iocvar.eq.1.or.  &
      ...
      call RAMS_fill_fld (idims(1),idims(2),idims(3)  &
                         ...
                         ,cdname(2),cdunits(2),iovtype,icoor,topta)
...
if(action.eq.'SPACE') then
   ...
   if(cwinds.ne.'n') then
      ...
      if(cwinds.eq.'t') then
         ...
         if(if_adap == 1) call RAMS_3to2d (islab,volt  &
                         ,coor,idims(1),idims(2),idims(3)  &
                         ,voltsl,scoor  &
                         ,nib,nie,njb,nje,icoor,nii,njj)
...
!*****************SBR**********************************************
   ...
   if(islab == 3.and.abs(mfill) > 0 .and. &
      (xtn(nie,nngd)-xtn(nib,nngd)) > 1000. .and.  &
      (ytn(nje,nngd)-ytn(njb,nngd)) > 1000.      ) then
      ibugout=0
   ...
   call framdrw (horiz,vert,xmplot,xtplot,ymplot,ytplot  &
                ...
                ,zuv,zmn(nnzp(nngd)-1,nngd),ivtype,vioff,vjoff,if_adap  &
                ,voltsl)
...
subroutine pppplt(a,n1,n2,n3)

real a(n1,n2,n3)

do k=1,n3

call ezcntr(a(1,1,k),n1,n2)
